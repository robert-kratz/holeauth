 ...
 * success returns a `NextResponse.next()` that:
 *   1. Carries the new `Set-Cookie` headers so the rotated tokens are
 *      persisted in the browser on this same navigation.
 *   2. Forwards the new access cookie into the **current request's** `cookie`
 *      header via `NextResponse.next({ request: { headers } })` so that
 *      Server Components running in the same SSR pass can read the access
 *      token immediately — without this, `validateCurrentRequest()` would
 *      redirect to /login even though the refresh just succeeded.
 */
async function tryRotateViaSubrequest(
  ...
  try {
    ...
    // Merge the rotated cookie values into the upstream request's Cookie
    // header so the current SSR pass sees the new access token immediately.
    const existing = (req.headers.get('cookie') ?? '')
      .split(';')
      .map((s) => s.trim())
      .filter(Boolean);
    for (const raw of setCookies) {
      // Each Set-Cookie line is "name=value; Path=...; ..."
      const nameVal = (raw.split(';')[0] ?? '').trim();
      const name = (nameVal.split('=')[0] ?? '').trim();
      if (!name) continue;
      const idx = existing.findIndex((e) => e.startsWith(`${name}=`));
      if (idx >= 0) existing.splice(idx, 1, nameVal);
      else existing.push(nameVal);
    }

    const newRequestHeaders = new Headers(req.headers);
    newRequestHeaders.set('cookie', existing.join('; '));

    const next = NextResponse.next({ request: { headers: newRequestHeaders } });
